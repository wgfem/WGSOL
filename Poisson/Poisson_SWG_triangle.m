function [u,AD,b,ers,info] = Poisson_SWG_triangle(node,elem,pde,bdFlag,option)
% This is to solve the Poisson equation on triangle partition.
%
%   u = Poisson_SWG_triangle(node,elem,pde,bdFlag,option) produces the linear
%   SWG approximation of the Poisson equation
% 
%       -Delta u = f, 
...
%   
%   The function Poisson_SWG_triangle assembes the matrix equation AD*u = b 
...
errordu=h*h*((DU(:,1)-DUIc(:,1)).*(DU(:,1)-DUIc(:,1)) + (DU(:,2)-DUIc(:,2)).*(DU(:,2)-DUIc(:,2)));
...
function [AD,b,u,freeDof,isPureNeumann]= getbdCCFVcoef_triangle(A,b)
    ...
    %% Part 1: Modify the matrix and b for Dirichlet condition
    % Only the case of Dirichlet Boundary condition is implemented
