function [u,AD,b,ers,info] = Poisson_SWG_octagon(node,elem,pde,bdFlag,option)
% This is to solve the Poisson equation on octagon partition.
%
%   u = Poisson_SWG_octagon(node,elem,pde,bdFlag,option) produces the linear
...
%   
%   The function Poisson_SWG_octagon assembes the matrix equation AD*u = b 
...
Elemo =[elem(:,1) N+elem2edge(:,4) elem(:,2) N+elem2edge(:,1) elem(:,3) N+elem2edge(:,2) elem(:,4) N+elem2edge(:,3)];
Edge1 =[edge(:,1) N+(1:NE)'];
Edge2 =[N+(1:NE)' edge(:,2)];
Edgeo =[Edge1;Edge2];
...
function [AD,b,u,freeDof,isPureNeumann]= getbdCCFVcoef_octa(A,b)
    ...
    %% Part 1: Modify the matrix an b for Dirichlet condition
