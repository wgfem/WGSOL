function [Nodeh,Elemh, u,AD,b,ers,info] = Poisson_SWG_hexagon(node,elem,pde,bdFlag,option)
% This is to solve the Poisson equation on hexagon partition.
%
%   u = Poisson_SWG_hexagon(node,elem,pde,bdFlag,option) produces the linear
...
%   
%   The function Poisson_SWG_hexagon assembes the matrix equation AD*u = b 
...
% Modify the matrix
    ...
    if ~isPureNeumann && ~isempty(fixedNode) && ~isempty(pde.g_D)
        if isnumeric(pde.g_D)  % pde.g_D could be a numerical array  
            ...
            u_fix1 = pde.g_D(0.5*(Nodeh(Edge1(fixeEdgeIndex,1),:) + Nodeh(Edge1(fixeEdgeIndex,2),:)));
            u_fix2 = pde.g_D(0.5*(Nodeh(Edge2(fixeEdgeIndex,1),:) + Nodeh(Edge2(fixeEdgeIndex,2),:)));
